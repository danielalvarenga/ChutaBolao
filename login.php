...
if(isset($_POST["acesso"])){
	...
	if(empty($_POST['username'])){
		echo "<script> alert('\"Nome de Usu·rio\" obrigatorio!')
		location = ('login.php');
		</script>";
	}
	elseif(empty($_POST['senha'])){
			echo "<script> alert('\"Senha\" obrigatorio!')
			location = ('login.php');
			</script>";
	}
	else{
		...
		$admin = $entityManager->find("Admin", $username);
		if($admin instanceof Admin){
			if($senha != $admin->getSenha()){
				echo '<html><body>
				<p align="center">A senha est· incorreta.</p>
				</body></html>';
			}
			else{
			...
			echo '<html><body>
					<p align="center">Usu·rio n„o encontrado. Solicite acesso ao Administrador.</p>
				</body></html>';
		}
...
<form id="login" method="post" action=""> 
    ...
    <h1>Painel de Administra√ß√£o <strong>Chuta Bol√£o</strong></h1>
    
    <div>
    	<label for="login_username">Nome de Usu√°rio</label> 
    	<input type="text" name="username" id="login_username" class="field required" title="Informe seu nome de usu√°rio" />
    </div>			

    <div>
    	<label for="login_password">Senha</label>
    	<input type="password" name="senha" id="login_senha" class="field required" title="Senha pbrigat√≥ria" />
    </div>			
